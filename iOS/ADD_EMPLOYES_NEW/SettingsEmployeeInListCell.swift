//  SettingsEmployeeInListCell.swiftt

import UIKit

class SettingsEmployeeInListCell: UITableViewCell {
    ...
    @IBAction func btnDeleteEmployee(_ sender: UIButton) {
        let alert = UIAlertController(title: "", message:
            "ARE_YOU_SURE_YOU_WANT_TO_DELETE_THE_EMPLOYEE".localized(LanguageMain.sharedInstance.USERLANGUAGE), preferredStyle: UIAlertController.Style.alert)
        alert.addAction(UIAlertAction(title: "OK".localized(LanguageMain.sharedInstance.USERLANGUAGE), style: .default, handler: { (action: UIAlertAction!) in
           let ax = Global.sharedInstance.ARRAYEMPLOYEE[self.tag].objsers.iUserId
           self.delegate.deleteEmployeaddtoArray(_whichID: ax)
           Global.sharedInstance.ARRAYEMPLOYEE.remove(at: self.tag)
