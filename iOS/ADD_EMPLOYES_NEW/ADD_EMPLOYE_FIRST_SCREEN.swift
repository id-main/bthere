...
class ADD_EMPLOYE_FIRST_SCREEN: UIViewController, UIGestureRecognizerDelegate, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource, saveInGlobalDelegate2 {
  ...
  //      OPENTABLE2_BUTTON.backgroundColor = UIColor.lightGray
        ...
        if ACCESS_PERMISSIONS_ARRAY_SELECTED.count == 0 {
            ...
            selectedPermission =  ACCESS_PERMISSIONS_ARRAY_SELECTED.joined(separator: ", ")
            SELECTED_PERMISSION_TEXT = selectedPermission
        ...
        if SERVICE_PERMISSIONS_ARRAY_SELECTED.count == 0 {
            ...
            selectedPermission =  SERVICE_PERMISSIONS_ARRAY_SELECTED.joined(separator: ", ")
            SELECTED_SERVICE_PERMISSION_TEXT = selectedPermission
                    ...
                    if let _ = RESPONSEOBJECT["Error"]!["ErrorCode"] as? Int {
                        if RESPONSEOBJECT["Error"]!["ErrorCode"] as! Int == 1
                        {
                            ...
                            if let _:Array<Dictionary<String,AnyObject>> = RESPONSEOBJECT["Result"] as?  Array<Dictionary<String,AnyObject>>
                            {
                                ...
                                let lastelement = self.APPPERMISSIONS_ARRAY.count
                                if let _:String = self.APPPERMISSIONS_ARRAY[lastelement - 1].nvValue {
                                    let preselected = self.APPPERMISSIONS_ARRAY[lastelement - 1].nvValue
                                    if let _:Int = self.APPPERMISSIONS_ARRAY[lastelement - 1].iPermissionType {
                                        let myid = self.APPPERMISSIONS_ARRAY[lastelement - 1].iPermissionType
                                        if !self.ACCESS_PERMISSIONS_ARRAY_TO_SERVER.contains(myid!) {
                                            self.ACCESS_PERMISSIONS_ARRAY_TO_SERVER.append(myid!)
                                        }
                                    }
                                    if !self.ACCESS_PERMISSIONS_ARRAY_SELECTED.contains(preselected!) {
                                        self.ACCESS_PERMISSIONS_ARRAY_SELECTED.append(preselected!)
                                        self.SELECTED_PERMISSION_TEXT = ""
                                        self.SELECTED_PERMISSION_TEXT_LABEL.text = ""
                                        selectedPermission =  self.ACCESS_PERMISSIONS_ARRAY_SELECTED.joined(separator: ", ")
                                        self.SELECTED_PERMISSION_TEXT = selectedPermission
                                        self.SELECTED_PERMISSION_TEXT_LABEL.text = self.SELECTED_PERMISSION_TEXT
                                    }
                                }
                                // 2more
                                if self.APPPERMISSIONS_ARRAY.count == 5 {
                                if let _:String = self.APPPERMISSIONS_ARRAY[3].nvValue {
                                    let preselected = self.APPPERMISSIONS_ARRAY[3].nvValue
                                    if let _:Int = self.APPPERMISSIONS_ARRAY[3].iPermissionType {
                                        let myid = self.APPPERMISSIONS_ARRAY[3].iPermissionType
                                        if !self.ACCESS_PERMISSIONS_ARRAY_TO_SERVER.contains(myid!) {
                                            self.ACCESS_PERMISSIONS_ARRAY_TO_SERVER.append(myid!)
                                        }
                                    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView.tag == 1 {
            ...
            let appPerm:appPermissions = APPPERMISSIONS_ARRAY[indexRow]
            if let _ =  appPerm.nvValue as String?  {
                let mybusinessdomain =  appPerm.nvValue as! String
            ...
            var isselected = false
            let myObjectforService = ProviderServicesArray[indexRow]
    ...
    func selectedDomain(_WHICHDOMAIN:Int, _state: Bool) {
        ...
        TABLE_SERVICE_PERMISSIONS.isHidden = true
        print(_WHICHDOMAIN)
        let appPerm:appPermissions = APPPERMISSIONS_ARRAY[_WHICHDOMAIN]
        var selectedPermission = appPerm.nvValue
        let myid = appPerm.iPermissionType

        if _state == false {
            if ACCESS_PERMISSIONS_ARRAY_TO_SERVER.contains(myid!) {
                let indexPermission = ACCESS_PERMISSIONS_ARRAY_TO_SERVER.index(of: myid!)
                ACCESS_PERMISSIONS_ARRAY_TO_SERVER.remove(at: indexPermission!)
                if let _ = ACCESS_PERMISSIONS_ARRAY_SELECTED[indexPermission!] as String?  {
                    ACCESS_PERMISSIONS_ARRAY_SELECTED.remove(at: indexPermission!)
                }
            }
        } else{
            ...
            if !ACCESS_PERMISSIONS_ARRAY_TO_SERVER.contains(myid!) {
                ...
                ACCESS_PERMISSIONS_ARRAY_SELECTED.append(selectedPermission!)
            }
        }
        if ACCESS_PERMISSIONS_ARRAY_SELECTED.count == 0 {
            SELECTED_PERMISSION_TEXT_LABEL.text = "CHOOSE_PERMISSION".localized(LanguageMain.sharedInstance.USERLANGUAGE)
    ...
    func selectedDomainTwo(_WHICHDOMAIN:Int, _state: Bool) {
        ...
        TABLE_ACCESS_PERMISSIONS.reloadData()
        TABLE_ACCESS_PERMISSIONS.isHidden = true
        var selectedPermission = "" // SERVICE_PERMISSIONS_ARRAY[_WHICHDOMAIN]""
        let myObjectforService = ProviderServicesArray[_WHICHDOMAIN]
        if let _ =  myObjectforService.nvServiceName as String?  {
            let selectedPermission =  myObjectforService.nvServiceName
            let myid = myObjectforService.iProviderServiceId
            if _state == false {
                if SERVICE_PERMISSIONS_ARRAY_TO_SERVER.contains(myid) {
                    let indexSrvPermission = SERVICE_PERMISSIONS_ARRAY_TO_SERVER.index(of: myid)
                    SERVICE_PERMISSIONS_ARRAY_TO_SERVER.remove(at: indexSrvPermission!)
                    if let _ = SERVICE_PERMISSIONS_ARRAY_SELECTED[indexSrvPermission!] as String?  {
                        let selectedSrvPermission = SERVICE_PERMISSIONS_ARRAY_SELECTED[indexSrvPermission!]
                        let indexSelectedSrvPermission = SERVICE_PERMISSIONS_ARRAY_SELECTED.index(of: selectedSrvPermission)
                        SERVICE_PERMISSIONS_ARRAY_SELECTED.remove(at: indexSelectedSrvPermission!)
                    }
                }
            } else{
                ...
                if !SERVICE_PERMISSIONS_ARRAY_TO_SERVER.contains(myid) {
                    ...
                    SERVICE_PERMISSIONS_ARRAY_SELECTED.append(selectedPermission)
                }
            }
        }
        if SERVICE_PERMISSIONS_ARRAY_SELECTED.count == 0 {
    ...
    func checkPhoneExist()
    {
        ...
        if phonenumber == myuserphonenumber {
            ...
            var dictionaryForServer:Dictionary<String,AnyObject> = Dictionary<String,AnyObject>()
            dictionaryForServer["nvPhone"] = TXTPHONENUMBER.text as AnyObject
            if Reachability.isConnectedToNetwork() == false
            {
    ...
    func CheckEligibleEmployee(){
        ...
        if phonenumber == myuserphonenumber {
            ...
            {
                api.sharedInstance.CheckEligibleEmployee(dictionaryForServer, success:{ (operation: AFHTTPRequestOperation!,responseObject: Any?) -> Void in
                    if let _ = responseObject as? Dictionary<String,AnyObject> {
                        let RESPONSEOBJECT = responseObject as! Dictionary<String,AnyObject>
                        ...
                        if let _ = RESPONSEOBJECT["Error"]!["ErrorCode"] as? Int {
                            if RESPONSEOBJECT["Error"]!["ErrorCode"] as! Int == 1
                            {
                                ...
                                if RESPONSEOBJECT["Error"]!["ErrorMessage"] as! String == "Phone does not exists in DB"
                                {
                                    self.ERRORTXTPHONENUMBER.text = "PHONE_NOT_EXIST_EMPLOYEE".localized(LanguageMain.sharedInstance.USERLANGUAGE)
                                }
                                else if RESPONSEOBJECT["Error"]!["ErrorMessage"] as! String == "Not eligible as employee"
                                {
                                    self.ERRORTXTPHONENUMBER.text = "NOT_ELIGIBLE_AS_EMPLOYEE".localized(LanguageMain.sharedInstance.USERLANGUAGE)
                                }
                                else if RESPONSEOBJECT["Error"]!["ErrorMessage"] as! String == "Already someones Employee"
                                {
                                    self.ERRORTXTPHONENUMBER.text = "ALREADY_TAKEN_EMPLOYEE".localized(LanguageMain.sharedInstance.USERLANGUAGE)
                                }
                                else
                                {
                                    self.ERRORTXTPHONENUMBER.text = "NOT_ELIGIBLE_AS_EMPLOYEE".localized(LanguageMain.sharedInstance.USERLANGUAGE)
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        if textField == TXTFULLNAME
        {
            if TXTFULLNAME.text != "" && isValidName(TXTFULLNAME.text!) == false
            {
                ...
                for element in fullNameArr {
                    print("eeeee |\(element.count)|")
                }
                if(size > 1) {
