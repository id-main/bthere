    ...
    var cropSize: CGSize {
        ...
        set {
            if let view = self.cropOverlayView {
                ...
                view.cropSize = CGSize(width: width, height: height)//newValue
            } else {
...
//        var height = bounds.size.height
        ...
        let cg: CGRect = CGRect(x: 0, y: 20,width: bounds.width, height: bounds.height-40)
                self.cropOverlayView.frame = cg// self.bounds
    ...
    func croppedImage() -> UIImage {
        ...
        if imageRef != nil {
         result = UIImage(cgImage: imageRef!, scale: imageToCrop!.scale,
    ...
    fileprivate func calcVisibleRectForResizeableCropArea() -> CGRect {
        ...
        let dragBall = UIPanGestureRecognizer(target: self, action:#selector(self.dragBall(_:)))
        self.addGestureRecognizer(dragBall)
        return visibleRect
    }
    @IBAction func dragBall(_ recognizer: UIPanGestureRecognizer) {
        let point = recognizer.location(in: self);
        self.cropOverlayView.center.x=point.x
        self.cropOverlayView.center.y=point.y
    }
