...
// label setting
let HRToastMaxWidth       :  CGFloat  = 0.9;      // 90% of parent view width
let HRToastMaxHeight      :  CGFloat  = 0.9;
...
let HRToastCornerRadius   : CGFloat   = 2.0
...
extension UIView {
    ...
    class func hr_toastThemeColor() -> UIColor {
        ...
        if color == nil {
            color = UIColor.black
           // color = Colors.sharedInstance.color4
            let semi = color!.withAlphaComponent(0.7)
            UIView.hr_setToastThemeColor(color: semi)
        }
        return color!
    }
