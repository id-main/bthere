//
//  SupplierContactSyncViewController.swift
//  Bthere
//
...
import Contacts
protocol hideChooseAllDelegatea {
    func hideChooseAll()
}
//דף 5 בהרשמה סנכרון עם אנשי קשר

class SupplierContactSyncViewController: NavigationModelViewController ,UITableViewDelegate,UITableViewDataSource,hideChooseAllDelegatea,UITextFieldDelegate {
   ...
   //\\ var existingcontactListFORSAVE:NSMutableArray = NSMutableArray() //precompleted on didload
    ...
    @IBAction func btnsearch(_ sender: AnyObject?) {
        ...
        if textX != "SEARCH_CONTACT".localized(LanguageMain.sharedInstance.USERLANGUAGE) &&  textX.count > 0  {
            ...
            issearch = true //let predicate = NSPredicate(format: "organizationName == %@", orgName)
            let namePredicate =
    ...
    func trysendingcontacts (_ objSyncContactsUsers:NSMutableArray) {
        ...
        api.sharedInstance.SyncContacts(dicomposed, success:{ (operation: AFHTTPRequestOperation!,responseObject: Any?) -> Void in
            self.generic.hideNativeActivityIndicatorInteractionEnabled(self)
...
//        }
    ...
    @IBAction func btnsave(_ sender: UIButton) {
        var phones:Array<String> = []
        btnsave.isUserInteractionEnabled = false // to prevend array out of bonds
        let dateString = "01/01/1901" // change to your date format
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat =  "dd/MM/yyyy"
        let birthday = dateFormatter.date(from: dateString)
...
//            }
    ...
    {
        let  cell = tableView.dequeueReusableCell(withIdentifier: "setContactManSyncTableViewCell")as!setContactManSyncTableViewCell
        cell.delegate = self
        cell.btnISsync.tag = indexPath.row
        cell.btnISsync.isUserInteractionEnabled = false
        if issearch == false {

            if contactList.indices.contains(indexPath.row) {
        ...
        //    print("nume :\(contactcheck.nvLastName) prenume : \(contactcheck.nvFirstName)nickname  \(contactcheck.nvNickName))")
            ...
            if searchcontactList.indices.contains(indexPath.row) {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if issearch == false {
         ...
         //   print("nume :\(contactcheck.nvLastName) prenume : \(contactcheck.nvFirstName)nickname  \(contactcheck.nvNickName))")
            ...
            if (tableView.cellForRow(at: indexPath)as!setContactManSyncTableViewCell).btnISsync.isHidden == true//כדי להדליק איש קשר
            {
                if phoneexists == false {
                    (tableView.cellForRow(at: indexPath)as!setContactManSyncTableViewCell).btnISsync.isHidden = false
            ...
            {
                if phoneexists == false {
                    (tableView.cellForRow(at: indexPath)as!setContactManSyncTableViewCell).btnISsync.isHidden = true
...
extension String
{
    func replacestr(_ target: String, withString: String) -> String
    {
