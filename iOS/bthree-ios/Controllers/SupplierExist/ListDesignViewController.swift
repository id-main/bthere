...
//דף ספק -  תצוגת רשימה
class ListDesignViewController: UIViewController,UITableViewDataSource,UITableViewDelegate ,openFromMenuDelegate,iCarouselDataSource, iCarouselDelegate,MFMailComposeViewControllerDelegate{
    //NEW DEVELOP
    ...
    override func viewDidLoad() {
        ...
        if let _:Int = USERDEF.value(forKey: "SHOWEYEINCALENDARSUPPLIER") as? Int {
            ...
            if y == 0 {
                ...
                btnSyncGoogleSupplier.isHidden = true
                nogoogle = true
            } else if y == 1 {
                btnSyncGoogleSupplier.isHidden = false
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
      ...
      var event:(String,Array<allKindEventsForListDesign>)?
        ...
        if sortDicBTHEREeventFiltered.indices.contains(indexPath.section) {
        ...
        {
            let cell1:RowInListTableViewCell = tableView.dequeueReusableCell(withIdentifier: "RowInListTableViewCell")as!RowInListTableViewCell
            cell1.dateTurn = dateturn
            cell1.isfromLIST = false
            cell1.selectionStyle = UITableViewCell.SelectionStyle.none
            cell1.sortArrEvnt = sortDicEvents
            cell1.event = event!.1[indexPath.row - 1]
            ...
            //-1
            if event!.1[indexPath.row - 1].iDayInWeek == -1 {
                isdayevent = true
            }
            if event!.1[indexPath.row - 1].fromHour == "16:03" {
        ...
        if   Global.sharedInstance.isSyncWithGoogelCalendarSupplier == true {
            for event in Global.sharedInstance.arrEvents
            {
                ...
                if (small(dateEvent!, rhs: dateIn5Month) == true && small(dateToday, rhs: dateEvent!) == true) || calendar.isDateInToday(dateEvent!) || (year5Month == yearEvent && month5Month == monthEvent && day5Month == dayEvent)
                {
                    ...
                    {
                        dicBthereEvent[dateFormatter.string(from: dateEvent!)]?.append(eventPhone)
                    }
                    else
                    {
    ...
    func TRYPDFJUSTNOW(_ NEWARRAY:[(String,Array<allKindEventsForListDesign>)]) {
        ...
        if NEWARRAY.count > 0 {
            ...
            for i in 0..<numberofsections {
                ...
                for z in 0..<y {
                    ...
                    if z == 0 {
                        ...
                        //                cell.setDisplayData(st, _desc:event!.1[indexPath.row - 1].ClientnvFullName,_EventFrom: event!.1[indexPath.row - 1].tag,_index: indexPath.section,_ClientDict: emi,_servicesdesc:event!.1[indexPath.row - 1].title, _isHourPast:isHourPast, _isOcasionalCustomer:isOcasionalCustomer)
                        //row 1 hours
                        if z % 18 == 0 {
            ...
            if( MFMailComposeViewController.canSendMail() )
            {
                ...
                {
                    print("File path loaded. \(tempDataPath)")
