//
//  ADD_CALENDAR_FIRST_SCREEN.swift
//  Bthere
//
...
class ADD_CALENDAR_FIRST_SCREEN: UIViewController, UIGestureRecognizerDelegate, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource, saveInGlobalDelegate2 {
    ...
    @IBAction func NEXTSTEP() {
        ...
        if SERVICE_PERMISSIONS_ARRAY_TO_SERVER.count == 0 {
        ...
        if self.isvalidname == true && self.isvalidphone == true  { //&& self.USERIDFROMSERVER > 0 {
            ...
            let viewEmployeeSettingsSetupWorkingHours: CalendarSettingsSetupWorkingHours = mainstoryb.instantiateViewController(withIdentifier: "CalendarSettingsSetupWorkingHours")as! CalendarSettingsSetupWorkingHours
...
//        btnNoSelect.isCecked = true
        ...
        OPENTABLE1_BUTTON.alpha = 1
        OPENTABLE2_BUTTON.alpha = 1
     //   let providerPhone = Global.sharedInstance.currentProviderDetailsObj.providerBuisnessDetailsObj.nvPhone
        let providerPhone = Global.sharedInstance.currentUser.nvPhone
        self.TXTPHONENUMBER.text = providerPhone
        self.isvalidphone = true
        self.TXTPHONENUMBER.isUserInteractionEnabled = false
        self.TXTPHONENUMBER.backgroundColor = UIColor.lightGray
        ...
        ADD_EMPLOYEE.text = "ADD_CALENDARS".localized(LanguageMain.sharedInstance.USERLANGUAGE)
    ...
    func selectedDomain(_WHICHDOMAIN:Int, _state: Bool) {
        ...
        if _state == false {
            ...
            if !ACCESS_PERMISSIONS_ARRAY_TO_SERVER.contains(myid!) {
                ...
                ACCESS_PERMISSIONS_ARRAY_SELECTED.append(permissionValue!)
            }
        }
        if ACCESS_PERMISSIONS_ARRAY_SELECTED.count == 0 {
            SELECTED_PERMISSION_TEXT_LABEL.text = "CHOOSE_PERMISSION".localized(LanguageMain.sharedInstance.USERLANGUAGE)
        } else {
            permissionValue = ACCESS_PERMISSIONS_ARRAY_SELECTED.joined(separator: ", ")

            SELECTED_PERMISSION_TEXT = permissionValue!
            SELECTED_PERMISSION_TEXT_LABEL.text = SELECTED_PERMISSION_TEXT
        }
        self.TABLE_ACCESS_PERMISSIONS.reloadData()
    }
    ...
    func CheckEligibleEmployee(){
        let phonenumber = TXTPHONENUMBER.text
        let currentUserPhoneNumber = Global.sharedInstance.currentUser.nvPhone
                        ...
                        if let _ = RESPONSEOBJECT["Error"]!["ErrorCode"] as? Int {
                            if RESPONSEOBJECT["Error"]!["ErrorCode"] as! Int == 1
                            {
                                ...
                                if let _:Dictionary<String,AnyObject> = RESPONSEOBJECT["Result"] as?  Dictionary<String,AnyObject> {
                                    let responeServer = RESPONSEOBJECT["Result"] as! Dictionary<String,AnyObject>
                                    print("validresponse \(responeServer)")
                                    if let _:Bool = responeServer["bIsEligible"] as? Bool {
                                        let valid:Bool =  responeServer["bIsEligible"] as! Bool
                                        if valid == true {
                                            self.isvalidphone = true
                                            let myemployes :Array<objEMPLOYEE> = Global.sharedInstance.ARRAYCALENDAR
                                            self.ERRORTXTPHONENUMBER.text = ""
                                            ...
                                            if let _:Int = responeServer["iCustomerUserId"] as? Int {
                                                let valid:Int =  responeServer["iCustomerUserId"] as! Int
                                                if valid > 0 {
                                                    self.USERIDFROMSERVER = valid
                                                    self.ERRORTXTPHONENUMBER.text = ""
                                                }
                                            }
                                            if let _:String = responeServer["nvFirstName"] as? String {
                                                let valid:String =  responeServer["nvFirstName"] as! String
                                                self.firstNameVerify = valid
                                            }
                                            if let _:String = responeServer["nvLastName"] as? String {
                                                let valid:String =  responeServer["nvLastName"] as! String
