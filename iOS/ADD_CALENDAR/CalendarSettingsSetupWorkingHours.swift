//
//  CalendarSettingsSetupWorkingHours.swift
//  Bthere
//
//  Created by Ioan Ungureanu on  29/03/19.
...
class CalendarSettingsSetupWorkingHours:UIViewController, UITableViewDelegate, UITableViewDataSource,isOpenRowDelegate8 {
    ...
    @IBAction func btnNoSelect(_ sender: AnyObject) {
        ...
        for workingHours in Global.sharedInstance.NEWARRAYOBJEMPLOYEEWORKINGHOURS {
            print("we work on \(workingHours.getDic())")
            for i in 1...7 {
                var hoursinday:Array<objWorkingHours> = []
                for index in 0..<Global.sharedInstance.NEWARRAYOBJEMPLOYEEWORKINGHOURS.count {
                    let hoursObj:objWorkingHours = Global.sharedInstance.NEWARRAYOBJEMPLOYEEWORKINGHOURS[index]
                    let ay = hoursObj.iDayInWeekType
                    if ay == i {
                        hoursinday.append(hoursObj)
                    }
    ...
    @IBAction func btnYesSelect(_ sender: AnyObject) {
        ...
        for i in 1...7 {
            ...
            for z in 0..<Global.sharedInstance.currentProviderDetailsObj.providerGeneralDetailsObj.arrObjWorkingHours.count {
                let objUser:objWorkingHours = Global.sharedInstance.currentProviderDetailsObj.providerGeneralDetailsObj.arrObjWorkingHours[z]
                let ay = objUser.iDayInWeekType
                if ay == i {
                    hoursinday.append(objUser)
                }
                if hoursinday.count == 0 {
                    //nothing to do
                } else  if hoursinday.count == 1 {
            ...
            for a in Global.sharedInstance.NEWARRAYOBJEMPLOYEEWORKINGHOURS {
                ...
                for i in 1...7 {
                    ...
                    for z in 0..<Global.sharedInstance.NEWARRAYOBJEMPLOYEEWORKINGHOURS.count {
                        let objUser:objWorkingHours = Global.sharedInstance.NEWARRAYOBJEMPLOYEEWORKINGHOURS[z]
                        let ay = objUser.iDayInWeekType
                        if ay == i {
                            hoursinday.append(objUser)
                        }
    ...
    override func viewDidLoad() {
        ...
        TitleScreen.text = "ADD_CALENDARS_HOURS".localized(LanguageMain.sharedInstance.USERLANGUAGE)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell:CalendarSettingsEditWorkingHoursCell = tableView.dequeueReusableCell(withIdentifier: "CalendarSettingsEditWorkingHoursCell") as! CalendarSettingsEditWorkingHoursCell
    ...
    @IBAction func gotoStepThree(_ sender:UIButton) {
        ...
        let viewCONFIRM_EMPLOYEE_DETAILS: CONFIRM_CALENDAR_DETAILS = mainstoryb.instantiateViewController(withIdentifier: "CONFIRM_CALENDAR_DETAILS")as! CONFIRM_CALENDAR_DETAILS
        ...
        viewCONFIRM_EMPLOYEE_DETAILS.delegate = self.delegate
        ...
        viewCONFIRM_EMPLOYEE_DETAILS.isNEW = true
        if Global.sharedInstance.employehassamehours == true {
