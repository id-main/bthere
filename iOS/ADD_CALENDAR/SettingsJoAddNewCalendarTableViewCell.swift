//
//  SettingsJoAddNewCalendarTableViewCell.swift
//  bthree-ios
//
//  Created byIoan Ungureanu on 29/03/2019
//  Copyright © 2019 Bthere. All rights reserved.
//

import UIKit

protocol openPopUpFromAddCalendarCell
{
    func addNewEmployee()
}


class SettingsJoAddNewCalendarTableViewCell: UITableViewCell,openPopUpFromAddCalendarCell {
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        lblAddEmployee.text = "ADD_CALENDAR".localized(LanguageMain.sharedInstance.USERLANGUAGE)
    }
    @IBAction func newEmployee(_ sender: UIButton) {
//        delegate.showPopupBeforeAddNewCalendar()
    ...
    func presentAccesCodePopup(WHICHSCREEN:Int) {
        ...
        if WHICHSCREEN == 2 {   //set calendar
            ...
            viewpopupAccessCode.delegateFromAddCalendarCell = self
            self.parentViewController?.present(viewpopupAccessCode, animated: true, completion: nil)
        }
