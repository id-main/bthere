...
open class LineChart: UIView {
 ...
 //       handleTouchEvents(touches, event: event!)
    ...
    fileprivate func drawLine(_ lineIndex: Int) {
        ...
        layer.strokeColor = Colors.sharedInstance.color4.cgColor //colors[lineIndex].CGColor
        layer.fillColor = nil
...
//        }
    ...
    fileprivate func drawXLabels() {
        ...
        for (index, _) in xAxisData.enumerated() {
            if index != (xAxisData.count - 1)
            {
            let xValue = self.x.scale(CGFloat(index)) + x.axis.inset - (width / 2)
                // I add + x.axis.inset , to the x
                //check it!
            let label = UILabel(frame: CGRect(x: xValue + x.axis.inset, y: y, width: width, height: x.axis.inset))
            label.font = UIFont (name: "OpenSansHebrew-Regular", size: 14)//UIFont.preferredFontForTextStyle(UIFontTextStyleCaption2)
...
//            let label = UILabel(frame: CGRect(x: -24, y: yValue, width: y.axis.inset + 20 , height: y.axis.inset))
//            label.font = UIFont (name: "OpenSansHebrew-Regular", size: 14)//UIFont.preferredFontForTextStyle(UIFontTextStyleCaption2)
//            label.textAlignment = .right
//            label.text = String(Int(round(i))) + "Y"
//            self.addSubview(label)
//        }
    }
