...
//        afterImageView.contentMode = UIViewContentMode.scaleToFill
    ...
    public func setCarouselLayout(displayStyle:Int, pageIndicatorPositon:Int, pageIndicatorColor:UIColor?, describedTitleColor:UIColor?, layerColor:UIColor?, language:Int!) {
    ...
    fileprivate func scrollToImageView(_ scrollDirect:direction) {
        ...
        case .left:
            ...
            case .left:
                ...
                if currentIndex + 1 > images.count - 1 {
                    self.endMe()
                   // beforeImageView.image = images[0]
                } else {
                    beforeImageView.image = images[currentIndex + 1]
                }
                break
            ...
            case .right:
                ...
                if currentIndex - 1 < 0 {
                    afterImageView.image = images[images.count - 1]
                }else {
                    afterImageView.image = images[currentIndex - 1]
                }
                break
    ...
    fileprivate func handleHalfFullImageViewFrame(_ isScroll:Bool) {
        ...
        switch isScroll {
        case true:
            beforeImageView.frame = CGRect.init(x: scrollView.frame.size.width + 30, y: 0, width: scrollView.frame.size.width , height: scrollView.frame.size.height)
            afterImageView.frame = CGRect.init(x: scrollView.frame.size.width * 3 + 30, y: 0, width: scrollView.frame.size.width , height: scrollView.frame.size.height)
            break
        default:
            beforeImageView.frame = CGRect.init(x: scrollView.frame.size.width + 80, y: 20, width: scrollView.frame.size.width , height: scrollView.frame.size.height - 60)
            currentImageView.frame = CGRect.init(x: scrollView.frame.size.width * 2 + 30, y: 0, width: scrollView.frame.size.width, height: scrollView.frame.size.height - 60)
            afterImageView.frame = CGRect.init(x: scrollView.frame.size.width * 3 - 20, y: 20, width: scrollView.frame.size.width, height: scrollView.frame.size.height - 60)
            break
        }
    ...
    self.isUserInteractionEnabled = false
         self.stopScrollImageView()
        DispatchQueue.main.asyncAfter(deadline: .now() + 6.0, execute: {

            if let myViewController = self.parentViewController! as? TutorialScreenSlider {
                print(myViewController.title ?? "TutorialScreenSlider")
                myViewController.closeScreen()
            }
        })
