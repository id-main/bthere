...
override func viewWillAppear(_ animated: Bool) {
    ...
    @objc func openVideoProviderScreen() {
        ...
        let introVideoObj:IntroVideoPlayerScreen = mainstoryb.instantiateViewController(withIdentifier: "IntroVideoPlayerScreen")as! IntroVideoPlayerScreen
        introVideoObj.view.frame = CGRect(x: 0, y:64, width:self.view.frame.size.width, height: self.view.frame.size.height - 64)
        introVideoObj.modalPresentationStyle = .custom
        introVideoObj.whichvideo = 0
        self.present(introVideoObj, animated: true, completion: nil)
