...
//        self.present(alertview,animated:true,completion: nil)
    ...
    func skipScreen() {
        ...
        let afterVideoObj:AfterVideoPlayerScreen = mainstoryb.instantiateViewController(withIdentifier: "AfterVideoPlayerScreen")as! AfterVideoPlayerScreen
        //    abc.view.frame = CGRect(x: 0, y:0, width:self.view.frame.size.width, height: self.view.frame.size.height)
        afterVideoObj.view.frame = CGRect(x: 0, y:64, width:self.view.frame.size.width, height: self.view.frame.size.height - 64)
        afterVideoObj.modalPresentationStyle = .custom
        self.present(afterVideoObj, animated: true, completion: nil) //GettoKnowtheSystemScreen
        } else {
