//
//  SettingsEditWorkingHoursCell.swift
//  bthree-ios
//
//  Created by Ioan Ungureanu on 18/12/2018
...
class SettingsEditWorkingHoursCell: UITableViewCell {
    let calendar = Foundation.Calendar.current
    var dateFormatter = DateFormatter()
    var outputFormatter: DateFormatter = DateFormatter()
    var delegate:isOpenRowDelegate1!=nil
    ...
    //    @IBAction func EditHoursButton(_ sender:UIButton) {
    //        // 1. populate first with already selected
    //        // 2. populate second with already selected
    //        // 3. put in array
    //        // delegate.savedDataOn(_WHICHCELL: self.indexSection, _WHICHSTATE: true)
    //        addHoursPressed = false
    //        delegate.openRow(_WHICHCELL: self.indexSection, _WHICHSTATE: true)
    ...
    override func awakeFromNib() {
        ...
        AddHoursButton.setTitle("AddHoursButton_TEXT".localized(LanguageMain.sharedInstance.USERLANGUAGE), for: UIControl.State())
        ...
        AddHoursButton.layer.cornerRadius = 0
        //        EditHoursButton.setTitle("EditHoursButton_TEXT".localized(LanguageMain.sharedInstance.USERLANGUAGE), for: UIControlState())
        //        EditHoursButton.titleLabel?.textColor = Colors.sharedInstance.color3
        //        EditHoursButton.layer.borderColor = Colors.sharedInstance.color3.cgColor
        //        EditHoursButton.layer.borderWidth =  1
        SAVEAddHoursButton.setTitle("SAVEAddHoursButton".localized(LanguageMain.sharedInstance.USERLANGUAGE), for: UIControl.State())
        ...
        if addHoursPressed == false && addSecondHoursPressed == false  {
            ...
            if (_firstinterval.nvFromHour != "00:00:00" &&  _firstinterval.nvToHour !=  "00:00:00") || (_firstinterval.nvFromHour == "00:00:00" &&  _firstinterval.nvToHour !=  "00:00:00") {
                ...
                if (_secondinterval.nvFromHour != "00:00:00" &&  _secondinterval.nvToHour !=  "00:00:00") || (_secondinterval.nvFromHour == "00:00:00" &&  _secondinterval.nvToHour !=  "00:00:00"){
                    ...
                    mydatef.dateStyle = .none
                    mydatef.dateFormat = "HH:mm:00"
                    let fromHour =  mydatef.date(from: _secondinterval.nvFromHour)!
                    let toHour =  mydatef.date(from: _secondinterval.nvToHour)!
                    let dateFormatter = DateFormatter()
                    dateFormatter.timeStyle = .none
                    dateFormatter.dateFormat = "HH:mm"
                ...
                if  USERDEF.integer(forKey: "CHOOSEN_LANGUAGE") == 0 {
                    ...
                    dtToHour.minimumDate = min
                    dtToHour.maximumDate = max
                    dtToHour.setDate ( mydatef.date(from: "08:00:00")!, animated:  true )
                    dtFromHour.setDate ( mydatef.date(from: "20:00:00" )!, animated:  true )
                    dtToHour.date =  mydatef.date(from: "08:00:00")!
                    dtFromHour.date =  mydatef.date(from: "20:00:00")!
                    dtFromHour.reloadInputViews()
                    dtToHour.reloadInputViews()
                    let dateFormatter = DateFormatter()
                    dateFormatter.timeStyle = .none
                    ...
                    mydatef.dateStyle = .none
                    mydatef.dateFormat = "HH:mm:00"
                    let min = mydatef.date(from: "00:00:00")!      //createing min time
                    let max = mydatef.date(from: "23:59:00") //creating max time
                    dtFromHour.minimumDate = min  //setting min time to picker
                    dtFromHour.maximumDate = max
                    dtToHour.minimumDate = min
                    dtToHour.maximumDate = max
                    dtToHour.setDate ( mydatef.date(from: "20:00:00")!, animated:  true )
                    dtFromHour.setDate ( mydatef.date(from: "08:00:00")!, animated:  true )
                    dtToHour.date =  mydatef.date(from: "20:00:00")!
                    dtFromHour.date =  mydatef.date(from: "08:00:00")!
                    dtFromHour.reloadInputViews()
                    dtToHour.reloadInputViews()
                    let dateFormatter = DateFormatter()
                    dateFormatter.timeStyle = .none
 ...
 ////////////////////////    precompletehoursfromsecond      ////////////////////
            ...
            if (_firstinterval.nvFromHour != "00:00:00" &&  _firstinterval.nvToHour !=  "00:00:00") || (_firstinterval.nvFromHour == "00:00:00" &&  _firstinterval.nvToHour !=  "00:00:00") {
                ...
                mydatef.dateStyle = .none
                mydatef.dateFormat = "HH:mm:00"
                let fromHour =  mydatef.date(from: _firstinterval.nvFromHour)!
                let toHour =  mydatef.date(from: _firstinterval.nvToHour)!
                let dateFormatter = DateFormatter()
                dateFormatter.timeStyle = .none
                dateFormatter.dateFormat = "HH:mm"
            ...
            if (_secondinterval.nvFromHour != "00:00:00" &&  _secondinterval.nvToHour !=  "00:00:00") || (_secondinterval.nvFromHour == "00:00:00" &&  _secondinterval.nvToHour !=  "00:00:00"){
                ...
                if  USERDEF.integer(forKey: "CHOOSEN_LANGUAGE") == 0 {
                    ...
                    mydatef.dateStyle = .none
                    mydatef.dateFormat = "HH:mm:00"
                    let min = mydatef.date(from: _firstinterval.nvToHour)!      // min time
                    let max = mydatef.date(from: "23:59:00")! // max time
                    dtFromHour.minimumDate = min  //setting min time to picker
                    dtFromHour.maximumDate = max
                    dtToHour.minimumDate = min
                    dtToHour.maximumDate = max
                    dtToHour.setDate ( mydatef.date(from: _secondinterval.nvFromHour)!, animated:  true )
                    dtFromHour.setDate ( mydatef.date(from: _secondinterval.nvToHour )!, animated:  true )
                    dtToHour.date =  mydatef.date(from: _secondinterval.nvFromHour)!
                    dtFromHour.date =  mydatef.date(from: _secondinterval.nvToHour)!
                    dtFromHour.reloadInputViews()
                    dtToHour.reloadInputViews()
                    ...
                    dtToHour.minimumDate = min
                    dtToHour.maximumDate = max
                    dtToHour.setDate ( mydatef.date(from: _secondinterval.nvToHour)!, animated:  true )
                    dtFromHour.setDate ( mydatef.date(from: _secondinterval.nvFromHour)!, animated:  true )
                    dtToHour.date =  mydatef.date(from: _secondinterval.nvToHour)!
                    dtFromHour.date =  mydatef.date(from: _secondinterval.nvFromHour)!
                    dtFromHour.reloadInputViews()
                    dtToHour.reloadInputViews()
                }
                let dateFormatter = DateFormatter()
        ...
        if addHoursPressed == false && addSecondHoursPressed == false {
            ...
            if (sender == dtFromHour) {
                ...
                if (Global.sharedInstance.defaults.integer(forKey: "CHOOSEN_LANGUAGE") == 0) {
                    ...
                    let aDate = dtFromHour.date
                    let bDate = dtToHour.date
